    ...
    public function guardar(): \Illuminate\Http\RedirectResponse
    {
        ...
        try {
            ...
            if ($model->getFillable()) {
                Diagnostico::create($payload);
            } else {
                $model->forceFill($payload)->save();
            }
        } catch (\Throwable $e) {
            report($e);
            return redirect(route('historias-clinicas.create', [$this->historia->paciente_id, $this->historia->cita_id]))->with('error', 'Error al crear Diagnóstico');
        }

        return redirect(route('historias-clinicas.create', [$this->historia->paciente_id, $this->historia->cita_id]))->with('success', 'Diagnóstico creado con éxito');
    }
}; ?>

<div>
    <x-slot name="header">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white text-center">Crear Diagnóstico</h1>
        <flux:menu.separator />
    </x-slot>
    ...
    <div class="max-w-full mx-auto p-6 bg-white dark:bg-zinc-900 rounded-lg shadow-md">
        <form wire:submit.prevent='guardar' enctype="multipart/form-data">
            ...
            <div class="flex justify-center">
                <x-action-button class="mt-2" label="Guardar Diagnóstico" variant="success" />
            </div>
        </form>
    </div>
</div>

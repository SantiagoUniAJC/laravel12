...
new class extends Component {
    ...
    public function guardar(): \Illuminate\Http\RedirectResponse
    {
        ...
        try {
            $model = new ExamenComplementario();
            if ($model->getFillable()) {
                ExamenComplementario::create($payload);
            } else {
                $model->forceFill($payload)->save();
            }
        } catch (\Throwable $e) {
            report($e);
            return redirect(route('historias-clinicas.create', [$this->historia->paciente_id, $this->historia->cita_id]))->with('error', 'Error al crear Examen Complementario');
        }

        return redirect(route('historias-clinicas.create', [$this->historia->paciente_id, $this->historia->cita_id]))->with('success', 'Examen Complementario creado con éxito');
    }
}; ?>

<div>
    <x-slot name="header">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white text-center">Crear Examen Complementario</h1>
        <flux:menu.separator />
    </x-slot>
    ...
    <div class="max-w-full mx-auto p-6 bg-white dark:bg-zinc-900 rounded-lg shadow-md">
        <form wire:submit.prevent='guardar' enctype="multipart/form-data">
            ...
            <div class="flex justify-center">
                <x-action-button class="mt-2" label="Guardar Examen Complementario" variant="success" />
            </div>
        </form>
    </div>
</div>
